...
show_help() {
    ...
    Prepare Postgres cluster for running TPC-H queries:
      ...
      * Copy additional postgresql configs from postgresql_after_prepare.conf
    ...
    -x create indexes, they are not created by default
    -a disable sanity checks: using Postgres built with assertions and
       wal_level_minimal
    -q generate queries, they are not generated by default
    -h display this help and exit

    Example:
    ./prepare.sh -s 2 -d /mnt/ramdisk/ars/mytpch-2
EOF
}
